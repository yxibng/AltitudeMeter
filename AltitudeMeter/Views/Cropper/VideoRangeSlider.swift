...
extension VideoRangeSlider {
    ...
    class Slider: UIView {
        ...
        private lazy var imageView: UIImageView = {
            ...
            return UIImageView(image: UIImage(systemName: imageName))
        }()
    }
...
class VideoRangeSlider: UIView {
    ...
    var onRangeUpdateCallback: ((VideoRangeSlider.Range, VideoRangeSlider.Slider.Direction) -> Void)?
    // call back
    ...
    private var minSpace: CGFloat {
        (self.bounds.size.width - 2 * Layout.sliderWidth) * minSpaceRatio
    }
...
extension VideoRangeSlider {
    ...
    @objc func onPanRightSlider(_ pan: UIPanGestureRecognizer) {
        ...
        let maxOffset = self.bounds.size.width - self.leftSlider.frame.maxX - minSpace - Layout.sliderWidth
        if newOffset < -maxOffset {
            newOffset = -maxOffset
        }
        if pan.state == .ended {

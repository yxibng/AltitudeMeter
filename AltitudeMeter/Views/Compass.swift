...
struct Triangle: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        if radius > rect.width / 2 {
            let offsetY = sqrt(
                radius * radius - rect.size.width * rect.size.width / 4
            )
...
struct Compass<Content: View>: View {
    ...
    private func centerCricleSize(geometry: GeometryProxy) -> CGSize {
        ...
        let radius =
        min(width, height) / 2 - triangleHeight - trianglePadding * 2
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                .onChange(of: degrees) { newDegrees in
                    // 计算最短路径
                    let diff = (newDegrees - currentDegrees).truncatingRemainder(dividingBy: 360)
                    let shortest = diff > 180 ? diff - 360 : diff
                    withAnimation(.easeInOut(duration: 0.5)) {
                        currentDegrees += shortest
                    }
                }

                makeContent()
                    .frame(
                        width: centerCricleSize(geometry: geometry).width,
